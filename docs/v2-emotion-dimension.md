...
- [ ] **情感重要性的定义** — intensity 高就算"情感上重要"？还是需要 LLM 额外判断"这个事件对用户的人生意义"？当前方案（LLM 标注 importance）是否合理，还是应该用 intensity 阈值替代？
- [ ] **情感触发关联的归属** — 是作为情感维度的一部分（emotion_triggers 表），还是放在静态知识池里作为"用户属性"？各自的优缺点？
- [ ] **主动检索的触发时机** — LLM 每次回复前都查情感数据？还是只在检测到情感内容时查？如何平衡响应延迟和情感感知质量？
