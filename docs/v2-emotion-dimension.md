...
多模态情感信号（语音语调、表情、生理信号等）统一映射到 VAD 空间后写入 emotion_events，通过 `source` 字段区分模态来源。上层消费方（L3 抽象、L4 叙事、L5 人格）无需感知模态差异。
...
对话中出现强烈情感表达（intensity ≥ 0.9 或 importance ≥ 0.9）→ 触发热路径晋升，不等睡眠周期。已在 `v2-architecture-discussion.md` §4-L0 热路径晋升中定义。
...
建立"话题/实体 → 当前情感 VAD"的映射，作为**人物画像的一部分**——描述一个人对某件事物当前的情感状态。主要消费场景：

| 场景 | 问题 | 数据需求 |
|:----|:-----|:---------|
| **人物画像** | "这个人对 X 是什么感觉？" | 当前情感 VAD + confidence |
| **情感回避** | "这个话题现在安全吗？" | 当前 valence + confidence |
| **情感记忆召回** | "这个话题关联过什么重要情感事件？" | 当前 VAD + 关联的 emotion_events |
| **L5 行为预测** | "当前情感状态会怎么影响行为选择？" | 当前 VAD（作为 GMM 上下文先验） |
...
emotion_triggers 的 `associated_vector` 是**当前情感状态**（人物画像的快照），不是历史总结。情感演化由 emotion_events 的时序数据覆盖：

```
人物画像（emotion_triggers）:
...
| confidence | 含义 | 消费行为 |
|:----------:|:-----|:---------|
| ≥ 0.7 | 稳定情感反应 | 正常使用 VAD 作为人物画像特征 |
| 0.3 ~ 0.7 | 情感在演化/不稳定 | 取最近 1 次 VAD（最新状态），标注低置信度 |
| < 0.3 | 数据不足或高度矛盾 | 不自动使用，交由消费方自行判断 |

### 6.4 LLM 对话中的使用方式

emotion_triggers 作为人物画像的一部分，LLM 在对话中自然使用：

```
用户输入 → LLM 检测到话题 X
    ↓
LLM 查询 emotion_triggers: "话题 X → 该用户 → 当前情感 VAD"
    ↓
if confidence >= 0.7:
    # 稳定情感反应，可以作为人物画像特征使用
    if valence < -0.6:
...
遗忘机制:
  → 情感数据参与冷热分级
  ← 各层冷却系数叠加影响情感热度
  ← importance 高（动态值大）→ 冷却减速（幂律衰减 §5.3）
```
