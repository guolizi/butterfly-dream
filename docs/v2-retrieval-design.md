...
- 只对**实际有信号**的层（factor ≠ 1.0）做几何平均
- 系数 < 1 → 冷却减速（保留在热区更久）
- 系数 > 1 → 冷却加速（更快冷却）

**为什么不用乘法：** 乘法随层数增加结果越来越极端。全部减速时 0.5×0.3×0.3=0.045（慢 22 倍），几何平均 = 0.356（慢 2.8 倍），保留信号的同时避免极端值。加速和减速自然抵消，无需魔法 clamp 阈值。
...
| 场景 | 乘法 | 几何平均 | 评价 |
|:----|:----|:---------|:-----|
| 全部加速（L1+L3） | 3.0 | 1.73 | 几何平均更温和 ✅ |
| 全部减速（L2+L4+L5） | 0.045 | 0.356 | 保留减速信号但不过度 ✅ |
| 混合（上例） | 0.45 | 0.82 | 信号清晰 ✅ |
| L4+L5 双减速 | 0.09 | 0.30 | 合理减速 ✅ |
| 单 L1 加速 | 1.5 | 1.50 | 单层时等价 ✅ |
