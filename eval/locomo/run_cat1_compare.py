#!/usr/bin/env python3
"""Run cat1 baseline eval (no cross-encoder)."""
...
def main():
    ...
    for i, qa in enumerate(cat1_qas):
        ...
        print(f"  [{i+1:2d}/{len(cat1_qas)}] Q{qid} score={score}{passed} ({n_facts} facts)")
        if score < 3:
            print(f"    GT: {str(ground_truth)[:50]}")
            print(f"    PR: {answer[:50]}")
    
    all_scores = [r["score"] for r in results]
    avg = sum(all_scores) / len(all_scores) if all_scores else 0
    acc = sum(1 for s in all_scores if s >= 4) / len(all_scores) * 100 if all_scores else 0
    print(f"\n  avg={avg:.2f} acc={acc:.1f}% ({sum(1 for s in all_scores if s >= 4)}/{len(all_scores)})")
    ...
    with open(out_path, "w") as f:
        json.dump({"results": results, "label": "BASELINE"}, f, indent=2, ensure_ascii=False)
