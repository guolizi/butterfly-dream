...
**Butterfly Dream** 是一个为 [Hermes Agent](https://hermes-agent.nousresearch.com) 设计的全维记忆插件，基于 Hermes 主线 **Holographic 分支**的 HRR 向量引擎和 SQLite 存储层。Holographic 分支在原版 Holographic（纯手动 `add_fact`）之上引入了 LLM 自动从对话中提取事实的基础能力；Butterfly Dream 在此基础上**全面扩展**：重要性评分、分类标签、三维检索、中文分词、实体关系图、事实合并、多媒体存储、反射/熔断/过滤等。从纯文本事实到图片/音频/视频，从单轮搜索到多跳推理，让 Agent 的记忆既有宽度又有深度——同时确保可靠性。
...
## 📊 功能溯源：Holographic 主线 vs Holographic 分支 vs Butterfly Dream

| 特性 | Holographic main | Holographic 分支 | **Butterfly Dream** |
|:-----|:---------------:|:---------------:|:------------------:|
| HRR 向量编码 | ✅ | ✅ | ✅ |
| SQLite 持久存储 | ✅ | ✅ | ✅ |
| 实体自动提取 (regex) | ✅ | ✅ | ✅ (增强 CJK 括号) |
| LLM 自动从对话提取事实 | ❌ | ✅ | ✅ |
| `action=add_fact` 手动添加 | ❌ | ✅ | ✅ |
| 实体关系图 | ❌ | ❌ | ✅ |
| 多跳推理 | ❌ | ❌ | ✅ |
| 重要性评分 (1-10) | ❌ | ❌ | ✅ |
| 分类标签 (category/tags) | ❌ | ❌ | ✅ |
...
| 实体摘要卡 (零 LLM 成本) | ❌ | ❌ | ✅ |
| 完整评测体系 (检索+提取) | ❌ | ❌ | ✅ |
| 代码行数 (估算) | ~400 | ~600 | ~1800+ |

## 🧪 评测集

Butterfly Dream 附带一个综合评测集，融合业界最佳实践评估记忆系统的各项能力。
...
# 提取评测 (真实 LLM 调用)
python3 eval/test_extraction.py                                     # 全量 20 场景

# 加载外部场景 (自动生成长度覆盖场景)
python3 eval/gen_long_scenarios.py --output eval/scenarios_gen.json   # 生成新场景
python3 eval/test_extraction.py --scenarios eval/scenarios_gen.json   # 评测

# 英文场景对比
python3 eval/test_extraction.py --scenarios eval/en_short_work.json   # 英文短场景
python3 eval/test_extraction.py --scenarios eval/en_med_work.json    # 英文中场景

# 模型对比
python3 eval/test_extraction.py --compare                            # 多模型对比
python3 eval/test_extraction.py --model gpt-4o-mini --provider openai # 指定模型
```

### 基线结果 (v1.0 — 公平评测)

所有 search 查询使用**自然语言问句**（如"用户最喜欢什么颜色？"），FTS5 使用 **OR 语义**生成候选，再由 HRR + Jaccard + 重要性进行三维评分排序。

| 指标 | 值 | 说明 |
|:-----|:--:|:-----|
| 场景数 | 18 | |
| 查询数 | 43 | |
| **R@1** | **0.764** | 自然语言问句大部分可正确命中 |
| R@3 | 0.907 | |
| 精确率 | 0.455 | OR 带来更多候选，部分噪声 |
| 平均延迟 | 8.88ms | 含完整 HRR + Jaccard 评分 |

**分能力表现：**

| 能力 | R@1 | R@3 | 说明 |
|:-----|:---:|:---:|:-----|
| 🔍 基础检索 (EN/ZH) | 0.600 | 1.000 | ✅ 英文含中文事实不匹配 |
| 🧬 实体探针 | 1.000 | 1.000 | ✅ 不受 FTS5 影响 |
| 🕰️ 时间线查询 | 0.333 | 1.000 | ✅ SQL 查询 |
| 🎯 矛盾检测 | 0.500 | 1.000 | ✅ 暴力对比 |
| 🌀 遗忘曲线 | 1.000 | 1.000 | ✅ OR + HRR 无衰减 |
| 🔗 联想记忆 | 0.800 | 1.000 | ⚠️ 同前缀事实排名竞争 |
| ✏️ 记忆编辑 | 0.500 | 0.750 | ⚠️ 旧事实 BM25 排名更高 |
| 🔄 同义改写 | 1.000 | 1.000 | ✅ 各种变体全命中 |
| 📊 大规模压力 | 0.600 | 0.800 | ⚠️ 公共 token 淹没问题 |

> 💡 **OR 语义 + 三维评分**：FTS5 改为 OR 后，自然语言问句中的任何 token 匹配都能生成候选，HRR 向量 + Jaccard + 重要性在候选上进行精确排序。R@1=0.764 是完整的检索管道（FTS5 → HRR → Jaccard → 衰减 → 重要性）协同工作的结果。
>
> 💡 **精确率偏低 (0.455)**：OR 语义会引入大量无关候选，HRR + Jaccard 能正确排序但无法完全过滤所有噪声。这是广度召回 vs 精度之间的固有 trade-off。
>
> 💡 **已知边缘情况**：英文问句查中文事实（0 匹配 token）、多事实共享公共 token 时的排名竞争、大规模下高频 token 淹没问题——均可通过调整候选池大小或引入 embedding 语义检索缓解。

> 💡 **遗忘曲线平坦**：在当前规模 (15 干扰事实) 下检索 R@1 保持 1.0，说明 FTS5 + 关键词 AND 查询在适度噪声下仍然可靠。在 10,000+ 事实级别才可能出现可测量的衰减。
>
> 💡 **改写一致性满分**：同一事实的不同关键词子集均能命中，说明 FTS5 索引覆盖完整。但自然语言问句（含事实未出现词语）不命中——这是 FTS5 基于关键词匹配的固有边界。
...
| 查询数 | 95 | 混合长度和领域 |
| **通过率** | **59/95 (62.1%)** | 端到端 LLM 提取+检索通吃 |
| **R@1** | **0.631** | 首条结果命中率 |
...
| 长度 | 通过率 | 场景数 | 说明 |
|:-----|:-----:|:------:|:-----|
| 🀄 内置 (3-5 轮) | **15/16 (93.8%)** | 5 | 精心设计的典型场景 |
| 📏 short (3-5 轮) | **16/18 (88.9%)** | 6 | 自动生成，含中文+英文 |
| 📐 medium (10-20 轮) | **13/30 (43.3%)** | 4 | 中等长度对话，LLM 遗漏细节 |
| 📜 long (30-50 轮) | 6/18 (33.3%) | 2 | 长对话遗漏增多，LLM 非确定性高 |
| 📚 very-long (50-100 轮) | 0/10 (0.0%) | 1 | JSON 解析失败（LLM 输出截断） |

> 💡 以上结果有 LLM 非确定性方差 (每次运行 ±5-15%)。英文场景受语言修复后从 0-1/8 提升至 **4/8**。

**英文场景表现：**

| 场景 | 通过率 | 说明 |
|:-----|:-----:|:-----|
| 🔤 [short] microservice migration | **3/3 ✅** | 语言修复后全通过 |
| 🔤 [medium] ecommerce architecture | **1/5** | 语言修复后英文事实，但LLM遗漏细节 |

> ✅ **语言对齐已修复**：提取提示词现根据对话语言自动切换输出语言（中文对话→中文事实，英文对话→英文事实）。英文查询不再因语言不匹配而全灭。技术术语（Docker/Kubernetes/GitHub Actions）在多语言场景下仍可跨语言命中。

### 与上游 Holographic (Holographic 分支) 对比

Butterfly Dream 在 Hermes 主线的 Holographic 分支基础上做了全面增强：

**Holographic 分支的改动**（Hermes 主线已有）：
- 引入 LLM 自动从对话中提取事实
- 添加 `action=add_fact` 手动添加事实接口
- 基础实体提取 (regex)

**Butterfly Dream 追加的特性**：
- 重要性评分 (1-10)：LLM 自动评估每条事实重要性
- 分类标签 (category/tags)：结构化记忆组织
- 三维检索 (HRR × Jaccard × 重要性 × 时效)：告别纯向量搜索
- 中文全面支持：jieba 分词、FTS5 中文索引与查询
- 实体关系图 + 多跳推理
- 异步触发 + 事实合并 + 跨源去重
- 持久标记 + 反射 (元认知) + 多媒体记忆
- 完整评测体系 + 多种对比基准
...
|:----|:---------------:|:--------------:|:-----|
| 🔤 英文自然语言查询 (8 条) | **0/8 (0%)** | **5/8 (62.5%)** | 见下方分析 |
| 🀄 中文自然语言查询 (8 条) | **0/8 (0%)** | **8/8 (100%)** | 见下方分析 |

**根因**：FTS5 默认 AND 语义 → 自然语言问句要求**所有词**出现在索引中 → 停用词（what/are/the/的/了/吗）导致全灭。Butterfly Dream 的 **OR 展开 + 前缀匹配 + jieba 分词** 让任意词匹配即可召回候选，再由三维评分精排。

> 💡 原版 Holographic 不论英文还是中文，面对自然语言问句都会因 FTS5 AND 语义 + 停用词问题全灭——它只能处理短关键词查询（如 "Python" "FastAPI"）。Butterfly Dream 的 OR 展开 + 前缀匹配 + jieba 分词是解决这个问题的关键。完整对比脚本见 `eval/en_compare.py`。
