...
> 💡 **持久过滤**：搜索时支持 `persistent_only=True` 参数，仅返回标记为持久（`is_persistent=true`）的长期有效事实，适用于跨会话召回核心配置和用户身份。`list` 操作也支持同名参数。

> 📅 **时间线查询**：`fact_store(action='timeline', entity='项目X')` 按 `created_at` 正序返回某实体的全部事实。支持 `min_importance` 过滤低重要性事实。适用于追踪架构决策变化、偏好演化等场景。
...
- **is_persistent** — 持久标记（`true` 跨会话长期有效，`false` 会话级临时上下文）

**持久判断规则：**
| `true` 跨会话有效 | `false` 临时上下文 |
|:-----------------|:------------------|
| 用户身份/角色/姓名 | "正在调试 X" |
| 核心架构/技术栈 | "构建当前失败" |
| 安全配置/服务器地址 | "今天的天气" |
...
用户手动通过 `memory(action='add', target='user')` 写入的记忆以及**反思**生成的元事实自动标记为持久。

提取后的事实自动经过**事实合并**管道（同实体归并 + 矛盾检测），确保库中无重复堆积。

同时，自动提取还会触发**跨源去重**：新事实与已有事实的 Jaccard 相似度 ≥ 0.7 时跳过写入，避免 LLM 提取与 `on_memory_write` 镜像生成重复。`fact_store(action='add')` 手动写入不受影响。
...
| 提取输出字段 | content + category + tags（手动 add） | ✅ 同上 + **importance 1-10 + is_persistent** |
| **⭐ 重要性评分** | ❌ | ✅ LLM 自动评分 1-10，检索加权 |
| **🏷️ 持久标记** | ❌ | ✅ LLM 自动判断持久性，`memory` 用户写入 / 反思自动标持久 |
| **🔇 琐事消息过滤** | ❌ | ✅ 跳过 "ok/好的/👍" 等无信息内容 |
| **🔗 跨源去重** | ❌ | ✅ LLM 提取 + on_memory_write + 反思自动去重 (Jaccard ≥ 0.7) |
| **⚡ 熔断保护** | ❌ | ✅ 连续 3 次失败后冷却 120 秒 |
| **🧠 反思 Reflection** | ❌ | ✅ 每 5 次提取后元分析 → 元事实 |
| **🧵 线程安全** | ❌ 无锁 | ✅ 锁保护共享状态 |
| **🧬 事实合并/冲突检测** | ❌ | ✅ 同实体自动归并 + ⚡矛盾检测 |
| **🎯 三维检索** | ❌ 仅 relevance × trust | ✅ Relevance × Recency × Importance + 场景权重 |
| **🕰️ 时效衰减** | ⚠️ 有代码，默认关闭 (`half_life: 0`) | ✅ **默认启用，可配半衰期** |
| CJK 实体提取改进 | ❌ | ✅ 混合脚本规则 + 停用词过滤 |
| FTS5 搜索修复 | ❌ | ✅ OR 组合语义修正 |
| **📅 时间线查询** | ❌ | ✅ 按实体按时间正序追踪变更历史 |
| **🖼️ 多媒体附件** | ❌ | ✅ 图片/音频/视频 CAS 存储 + FTS5 |
