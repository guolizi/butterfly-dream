...
所有 search 查询使用**自然语言问句**（如"用户最喜欢什么颜色？"），不迁就被测系统的检索策略。评测结果如实反映系统能力：

| 指标 | 值 | 说明 |
|:-----|:--:|:-----|
| 场景数 | 18 | |
| 查询数 | 43 | |
| **R@1** | **0.066** | 自然语言搜索几乎全部失效 |
| R@3 | 0.093 | |
| 精确率 | 0.093 | |
| 平均延迟 | 0.31ms | |
...
| 能力 | 查询类型 | R@1 | 说明 |
|:-----|:--------:|:---:|:-----|
| 🔍 实体探针 | `probe` | 1.000 | ✅ 直接实体查找 |
| 🕰️ 时间线查询 | `timeline` | 0.333–1.000 | ✅ SQL 查询 |
| 🎯 矛盾检测 | `contradict` | 0.500 | ✅ 暴力对比 |
| 🗑️ 去重检测 | `dedup_check` | ✅ | ✅ 计数检查 |
| 🔍 **自然语言搜索** | `search` | **0.012** | ❌ **FTS5 无法处理自然语言问句** |

> 💡 **为什么自然语言搜索几乎全部失效？**
>
> 检索管道的流程是：`FTS5 候选生成 → HRR 评分 → Jaccard 评分 → 三维组合`。
> **FTS5 使用 AND 语义**——查询中的所有 token 都必须出现在同一条事实的 FTS5 索引中。
>
> 自然语言问句（如"用户最喜欢什么颜色？"）经 jieba 分词后得到 `["用户", "最喜欢", "什么", "颜色"]`，
> FTS5 查询为 `用户 AND 最喜欢 AND 什么 AND 颜色`。事实"用户最喜欢的颜色是蓝色"中有"用户""最喜欢""颜色"，
> 但**没有"什么"** → FTS5 返回 0 候选 → 管道提前终止，HRR 和 Jaccard 评分根本未执行。
>
> **这是系统当前的架构瓶颈**：FTS5 作为唯一的候选生成入口，无法处理含未知词的查询。
> 突破方向：添加语义检索（HRR 或 embedding）作为候选生成补充，让 FTS5 不命中的查询
> 仍可通过向量相似度找到相关事实。
