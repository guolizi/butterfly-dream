...
Butterfly Dream 会在以下两个时机**自动**调用 LLM 从对话历史中提取值得记忆的事实：
...
```
对话消息 → 截断保护(≤24K字符) → LLM 调用 → JSON 解析
...
| 重要性评分 | ❌ | ✅ LLM 自动评分 |
| **LLM 自动提取** | ✅ 压缩前 + 会话结束时异步调用 | ✅ **同上 + 带重要性评分(1-10)/分类/标签** |
| 时效衰减 | ⚠️ 可选（默认关闭） | ✅ 默认启用，可配置半衰期 |
| 实体图谱 | ✅ 实体解析 + HRR 代数推理 | ✅ **增强：实体关系图(edge表) + SQL 交叉检索** |
| 事实合并/冲突解决 | ❌ 无合并机制 | ✅ 同实体事实自动归并 + 矛盾检测 |
