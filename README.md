...
- **🔤 中文分词** — FTS5 全文搜索接入 jieba 分词引擎，混合中文/英文内容正确索引，bigram 补丁确保任意子串均可命中
...
1. **索引侧**：FTS5 sync trigger 调用 `jieba_segment()` SQLite 函数，在内容进入 FTS5 索引前，用 jieba 分词并在词间插入空格，同时补充 CJK bigram 作为细粒度 token。
2. **查询侧**：`_sanitize_fts_query()` 对用户查询同样进行 jieba 分词 + bigram 补丁，确保查询 token 与索引 token 匹配。

**效果**：
- `"喝咖啡"`（单个 jieba 词）→ 可搜索 `"喝咖啡"`、`"咖啡"`（bigram）、`"喝"`（bigram）
- `"用户喜欢喝咖啡和编程"` → jieba 分为 `["用户", "喜欢", "喝咖啡", "和", "编程"]` → 搜索 `"喜欢编程"` 命中
- 英中混合 `"love 猫咪"` → 同时搜索英文 token 和中文 bigram
...
| 特性 | Holographic main | Holographic feat/holographic-llm-extract | **Butterfly Dream** |
|---|---|---|---|
| HRR 向量编码 | ✅ | ✅ | ✅ |
| SQLite 持久存储 | ✅ | ✅ | ✅ |
| 实体自动提取 | ✅ (regex) | ✅ (regex) | ✅ (regex + CJK brackets) |
| 实体关系图 | ❌ | ❌ | ✅ |
| 多跳推理 | ❌ | ❌ | ✅ |
| LLM 自动提取 | ❌ | ✅ | ✅ |
| 重要性评分 (1-10) | ❌ | ❌ | ✅ |
