...
### 与上游 Holographic (原版) 对比

Butterfly Dream 在 Holographic 的 HRR 引擎和 SQLite 存储层基础上**全面增强**：

- **🗣️ LLM 自动提取** — LLM 在对话压缩和会话结束时自动分析对话、提取事实，带重要性/分类/标签
- **⭐ 重要性评分 (1-10)** — LLM 自动评估每条事实重要性
- **🏷️ 分类标签 (category/tags)** — 结构化记忆组织
- **🧠 三维检索** — HRR × Jaccard × 重要性 × 时效，告别纯向量搜索
- **🔤 中文全面支持** — jieba 分词、FTS5 中文索引与查询、OR 展开 + 前缀匹配
- **🔗 实体关系图 + 多跳推理**
- **🕰️ 时间感知 — 指数衰减时效**
- **📊 信任度反馈训练**
- **🔄 异步触发 + 事实合并 + 跨源去重**
